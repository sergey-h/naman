...
// Get all old redux options
$wpex_options = get_option( 'wpex_options' );

// Return if there aren't any options (first time installs )
if ( empty( $wpex_options ) ) {
...
// Get all redux sections and loop through them and saves them as theme_mods
function wpex_migrate_redux() {
	// Get theme options
	global $wpex_options;
	// Get old sections & loop through theme to set new theme mods
	$sections = wpex_legacy_redux_array();
	if ( $sections ) {
		foreach ( $sections as $section ) {
			...
			foreach( $fields as $field ) {
				...
				$val	= isset( $wpex_options[$id] ) ? $wpex_options[$id] : '';
...
/*-----------------------------------------------------------------------------------*/
/*	- Migrate the custom login options to new admin panel
/*-----------------------------------------------------------------------------------*/
function wpex_migrage_custom_admin_login_settings() {
	global $wpex_options;
	$options = $wpex_options;
...
// Transfer settings to the custom login admin page
wpex_migrage_custom_admin_login_settings();


/*-----------------------------------------------------------------------------------*/
/*	- Save option to prevent migration from running again
/*	- And clear the customizer cache
/*-----------------------------------------------------------------------------------*/
